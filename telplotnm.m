function h=telplotnm(INFILE,VAR,TIMESTEP,LIMS)

% Plots a patch for a TELEMAC2D triangular, finite element style mesh
% (i.e. a Seraphin format file), with NO MESH displayed! (telplotnm.m)
%
% usage:
%  h=telplotnm(INFILE,VAR,TIMESTEP,LIMS)
%
% where:
...
mesh = 0; %1 for on 0 for off  %Jon French July 2013
...
% plot the patch (with test for mesh display - Jon French addition)
if mesh == 1
 h=patch('faces',m.IKLE,'vertices',m.XYZ,'FaceVertexCData',m.RESULT(:,VAR), ...
    'FaceColor','interp','EdgeColor','k','linewidth',0.01);
elseif mesh ==0
 h=patch('faces',m.IKLE,'vertices',m.XYZ,'FaceVertexCData',m.RESULT(:,VAR), ...
    'FaceColor','interp','EdgeColor','none','linewidth',0.01);
else
 error('Mesh display flag must be set to either 1 or 0');
end
axis equal
axis tight
set(gca,'box','on')
